...
@section('title', 'Translate PDF to French Free — Traduire PDF en Français')
@section('description', 'Translate PDF to French online free. AI translation preserves formatting and structure. No account required. Perfect for students, researchers and businesses.')
@section('keywords', 'translate pdf to french, pdf to french translator, traduire pdf en francais, pdf french translation free, translate pdf french online')
@section('slug', 'translate-pdf-to-french')
@section('schema')
...
<div class="hero">
  <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;margin-bottom:20px;">
    <div class="badge">🇫🇷 PDF to French</div>
    <div class="badge" style="background:rgba(0,229,160,.1);border-color:rgba(0,229,160,.4);color:#00e5a0;">🆓 Free</div>
  </div>
  <h1>Translate PDF to French — Traduire PDF en Français</h1>
  <p>Upload any PDF and get an accurate French translation in seconds. AI preserves paragraph structure. No signup, no watermark, instant download.</p>
</div>
<div class="tool-box">
  <div class="upload-area" id="drop-zone" onclick="document.getElementById('pdfInput').click()">
    <div class="upload-icon">🇫🇷</div>
    <div class="upload-title">Drop your PDF here to Translate to French</div>
...
<div style="max-width:700px;margin:0 auto 60px;padding:0 20px;text-align:center;">
  ...
  <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center;">
    @foreach([['🇧🇩 Bengali','/pdf-translator-bengali'],['🇮🇳 Hindi','/translate-pdf-to-hindi'],['🇪🇸 Spanish','/translate-pdf-to-spanish'],['🇸🇦 Arabic','/translate-pdf-to-arabic'],['🇩🇪 German','/translate-pdf-to-german'],['🇧🇷 Portuguese','/translate-pdf-to-portuguese'],['🌐 All Languages','/translate-pdf']] as $l)
...
async function processTranslate() {
    ...
    let secs = 0; result.innerHTML = '<div style="color:#8888a8;padding:20px">⏳ Translating to French... <span id="timer">0s</span></div>';
    const iv = setInterval(() => { secs++; const t=document.getElementById('timer'); if(t) t.textContent=secs+'s'; }, 1000);
    const fd = new FormData(); fd.append('file', selectedFile); fd.append('language', 'French'); fd.append('_token', CSRF);
