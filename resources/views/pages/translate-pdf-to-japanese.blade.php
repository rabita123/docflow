...
@section('title', 'Translate PDF to Japanese Free — PDF日本語翻訳オンライン')
@section('description', 'Translate PDF to Japanese online free. AI-powered translation with accurate kanji and hiragana output. No signup, instant download.')
@section('keywords', 'translate pdf to japanese, pdf to japanese translator, pdf japanese translation free, translate pdf japanese, pdf 日本語 翻訳')
@section('slug', 'translate-pdf-to-japanese')
@section('schema')
...
<div class="hero">
  <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;margin-bottom:20px;">
    <div class="badge">🇯🇵 PDF to Japanese</div>
    <div class="badge" style="background:rgba(0,229,160,.1);border-color:rgba(0,229,160,.4);color:#00e5a0;">🆓 Free</div>
  </div>
  <h1>Translate PDF to Japanese — PDF日本語翻訳</h1>
  <p>AI-powered Japanese translation with proper kanji, hiragana, and katakana. No signup, instant download.</p>
</div>
<div class="tool-box">
  <div class="upload-area" id="drop-zone" onclick="document.getElementById('pdfInput').click()">
    <div class="upload-icon">🇯🇵</div>
    <div class="upload-title">Drop your PDF here to Translate to Japanese</div>
...
<div style="max-width:700px;margin:0 auto 60px;padding:0 20px;text-align:center;">
  ...
  <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center;">
    @foreach([['🇨🇳 Chinese','/translate-pdf-to-chinese'],['🇰🇷 Korean','/translate-pdf'],['🇮🇳 Hindi','/translate-pdf-to-hindi'],['🇪🇸 Spanish','/translate-pdf-to-spanish'],['🌐 All Languages','/translate-pdf']] as $l)
...
async function processTranslate(){
    ...
    let secs=0;result.innerHTML='<div style="color:#8888a8;padding:20px">⏳ 日本語に翻訳中... <span id="timer">0s</span></div>';
    const iv=setInterval(()=>{secs++;const t=document.getElementById('timer');if(t)t.textContent=secs+'s';},1000);
    const fd=new FormData();fd.append('file',selectedFile);fd.append('language','Japanese');fd.append('_token',CSRF);
